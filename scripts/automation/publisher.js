...
/**
 * Pexels에서 제공받은 외부 이미지 URL을 검증하고
 * 게시글에 사용할 이미지 메타데이터를 반환한다.
 *
...
function prepareArticleImages(article) {
  ...
  for (const image of article.images) {
    if (image?.provider !== "pexels") {
      throw new Error("지원하지 않는 이미지 제공자입니다.");
    }

    if (!image?.publicPath || !/^https:\/\/(?:images\.)?pexels\.com\//i.test(image.publicPath)) {
      throw new Error(`허용되지 않은 이미지 URL입니다: ${image?.publicPath || "없음"}`);
    }

    if (!image?.sourceUrl || !/^https:\/\/www\.pexels\.com\//i.test(image.sourceUrl)) {
      throw new Error(`Pexels 원본 URL이 올바르지 않습니다: ${image?.sourceUrl || "없음"}`);
    }
